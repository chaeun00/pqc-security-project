...
---
## Day 15 리뷰 (2026-03-27)

### 인수조건 달성
- ✅ 필수 필드 disabled
- ✅ Optimistic rollback (create/update/delete)
- ✅ AlgorithmSwitchPanel CbomPage 연결 (구현)
- ❌ AlgorithmSwitchPanel CbomPage 연결 테스트 미작성 (CbomPage.test.tsx)

### 주요 리스크
- MSW PUT 핸들러 응답값 idx 참조 불명확
- Optimistic ID Date.now() 충돌 가능성

### 테스트 공백
- 편집 성공/실패 케이스 누락
- CbomPage 전환 버튼 → Panel 렌더 테스트 누락
- CbomPage.test.tsx에 AlgorithmSwitchPanel 연결 케이스 1개 추가

---
## Day 16 — 실시간 모니터링 UI 리뷰 (2026-03-27)

### 변경 요약
- useMonitor.ts (신규): /api/cbom + refetchInterval:5000
- MonitorPage.tsx (수정): 통계 카드 4개, 알고리즘별 CSS bar, 최근 이력 5건, isError 배지
- MonitorPage.test.tsx (신규): 4케이스 (로딩·정상·isError·정렬)

### 위험 요소
- [위험-중] 테스트 실행 결과 미확인 (WSL 환경 문제)
- [위험-저] 로딩 테스트 flaky 가능성 (MSW 즉시 응답 시)
- [위험-저] isError 시 빈 테이블 동시 노출 (스펙 미정)
- 보안 취약점 없음

### 테스트 공백
- entries 빈 배열 케이스 미검증
- bar 너비(aria-label) 비율 미검증

### 판정
조건부 Day 17 진행 가능 — vitest all green + feat 커밋 후

---
## Review: Day 17 — Prometheus/Grafana 알람 연동 (2026-03-27)

### 인수조건 판정
| AC | 판정 |
|----|------|
| 1. Grafana 접근 + datasource green | ✅ 프로비저닝 파일 존재 |
| 2. Prometheus targets api-gateway UP | ✅ scrape job + actuator 노출 확인 |
| 3. PqcApiGatewayDown 알람 규칙 로드 | ✅ pqc_alerts.yml 정의됨 |

### 위험 요소
- [보안-중] GRAFANA_ADMIN_PASSWORD 기본값 "admin", pqc-public 노출 (docker-compose.yml:159)
- [보안-낮] Prometheus 9090 pqc-public 노출 — 인증 없이 메트릭 전체 열람 가능
- [기능-중] cbom_assets_total Counter 미정의 → PqcHighRiskAlgorithmDetected 알람 영원히 미발동

### 테스트 공백
- /metrics에 cbom_assets_total 미포함 검증 없음
- Grafana datasource health API 자동 검증 없음
- /api/v1/rules 알람 로드 assert 없음

---
## Day 18 — 컴포넌트 테스트 & E2E 리뷰 (2026-03-28)

### 변경 요약
vite.config.ts coverage 블록·임계값, package.json test:coverage/test:e2e 스크립트, @playwright/test + playwright.config.ts, E2E 시나리오 2개, CbomMetricsTest, EncryptControllerTest HIGH 경로, MonitoringIT 추가. 단, dashboard/src/** 변경 없음 — 컴포넌트 테스트 보완(Step A 후반) 수행 여부 미확인.

### 위험 요소 및 엣지 케이스
- [보안-HIGH] MonitoringIT.java:40 — admin:admin Base64 하드코딩 (Git history 영구 노출)
- [보안-MEDIUM] E2E 스펙 2개 — admin/password 평문 하드코딩
- [기능-MEDIUM] playwright.config.ts — MSW browser mock 미설정, CI에서 API 호출 실패 예상
- [기능-LOW] CbomMetrics.recordEncrypt — null/empty riskLevel 미검증

### 테스트 공백
1. npm run test:coverage 실행 결과 없음 (인수조건 ① 불명확)
2. npm run test:e2e 실행 결과 없음 (MSW 미설정으로 CI 실패 예상)
3. MonitoringIT — monitoring.integration=true CI 설정 보장 없음
4. CbomMetricsTest — LOW/null/"" 경계값 케이스 누락
5. EncryptControllerTest — LOW/MEDIUM 경로 cbomMetrics never() 검증 없음

### 수정 제안
1. MonitoringIT: 자격증명을 환경변수에서 동적으로 읽도록 변경
2. E2E 스펙: process.env.E2E_USER/E2E_PASS 환경변수로 분리
3. playwright.config.ts: MSW 또는 백엔드 연동 방식 확정
4. CbomMetricsTest: null/빈 문자열 케이스 추가

### 인수조건 판정
- ① test:coverage 70%+: 미확인
- ② test:e2e 2개 pass: 미확인 (MSW 불완전)
- ③ npm run build 0건: 미확인

Day 19 진행 전 권장: test:coverage 통과 확인 + MonitoringIT 자격증명 제거 + test:e2e 1회 통과 확인
---
## Day 20 — Phase 3 최종 검증 & 문서화 (2026-03-30)

### 변경 요약
`.env.test` 신규 생성 (환경변수 교체 기동 테스트용), `docs/plan.md` Day 20 인수조건 3개 [x] 체크, `README.md` Phase 3 완료 상태 반영 및 Quick Start에 `docker compose --env-file .env.test up -d` 커맨드 추가.

### Phase 3 인수조건 체크리스트

| # | 항목 | 판정 |
|---|------|------|
| A-1 | make up → POST /api/encrypt → GET /api/cbom → POST /api/cbom/manual (HTTP 200, DB row) | ✅ |
| A-2 | make test-dct → Cosign verify 전 이미지 통과 로그 + 미인증 /api/cbom → 403 | ✅ |
| A-3 | .env.test 교체 기동 → docker compose ps running + /actuator/health 200 | ✅ |

### Step B — 환경변수 교체 기동 테스트 절차
```bash
# .env.test 사용 기동
...
### 위험 요소 및 엣지 케이스
- [LOW] `.env.test`의 CRYPTO_ENGINE_HOST/DB_HOST/GATEWAY_HOST는 문서화 목적 — 실제 서비스 간 라우팅은 Docker 내부 서비스명으로 동작
- [LOW] `docker compose --env-file .env.test up -d`는 `pqc-db:secure` 이미지 사전 빌드(`make build-secure-db`) 필요

### 테스트 공백
- CI에서 `docker compose --env-file .env.test up -d` 연동 미포함 (로컬 검증만 수행)
- E2E 전 구간(POST /api/encrypt → CBOM 자동 기록) 자동화 스크립트 미존재

### 수정 제안
- 없음 (문서화·체크리스트 완결 목적, 코드 변경 불필요)

### 인수조건 판정
- ① make test-dct 종료코드 0: ✅
- ② docker compose --env-file .env.test 후 Up + /actuator/health 200: ✅
- ③ docs/plan.md 인수조건 3개 [x] + docs/review.md Day 20 Append: ✅
