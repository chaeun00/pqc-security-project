...
# Step 1-C rev.22: build-secure → 4서비스 순차 파이프라인
#
...
# ── 서비스 관리 ───────────────────────────────────────────
up: ## 전체 서비스 시작 (pqc-db:secure 이미지 필요, 없으면 make build-secure-db 실행)
	@docker image inspect pqc-db:secure > /dev/null 2>&1 || { \
		echo "Error: pqc-db:secure 이미지 없음. 먼저 'make build-secure-db'를 실행하세요."; \
		exit 1; \
	}
	$(COMPOSE) up -d db
...
#
#   Stage 2 — 미서명 이미지 (postgres:17-alpine, Docker Hub 공식, CVE-2025-68121 수정)
...
test-dct: ## [Step 1-A] Cosign keyless 이미지 서명 검증 (rev.7)
	...
	@echo "==> [Stage 2] 대상: postgres:17-alpine (미서명 이미지, Docker Hub 공식)"
	@echo "==> [2.1] 서명 없음 사전 확인 (pull 없이)"
	@if ! cosign verify postgres:17-alpine > /dev/null 2>&1; then \
	...
	@ERR=$$(cosign verify postgres:17-alpine 2>&1); RC=$$?; \
	if [ $$RC -ne 0 ]; then \
...
#   Step 1-A: db 베이스 이미지 (postgres:17-alpine, CVE-2025-68121 수정) 스캔
...
trivy: ## [Step 1-B] Trivy Critical 취약점 스캔 (postgres + crypto-engine)
	...
	@echo "==> Trivy 스캔: postgres:17-alpine"
	@trivy image --severity CRITICAL --exit-code 1 postgres:17-alpine
...
# ──────────────────────────────────────────────────────────
# build-secure-db — Step 1-C: db 서비스 보안 파이프라인
#   [1/3] cosign verify: postgres:17-alpine 베이스 서명 확인 (WARN-only)
#   [2/3] docker compose build db: Multi-stage su-exec COPY 이미지 빌드 (pqc-db:secure)
#   [3/3] trivy --vex: pqc-db:secure Critical 0건 게이트 (CVE-2025-68121 제거 + VEX)
# ──────────────────────────────────────────────────────────
build-secure-db: ## [Step 1-C] db: Cosign WARN + build(multi-stage su-exec) + Trivy+VEX 스캔
	@which cosign > /dev/null 2>&1 || { echo "Error: cosign 미설치. 'make setup' 실행."; exit 1; }
	@which trivy  > /dev/null 2>&1 || { echo "Error: trivy 미설치."; exit 1; }
	@echo "==> [build-secure-db 1/3] Cosign: postgres:17-alpine 베이스 서명 확인 (WARN-only)"
	@cosign verify postgres:17-alpine > /dev/null 2>&1 \
		&& echo "==> [build-secure-db 1/3] PASS ✓ — Cosign 서명 확인" \
...
#   [3/4] cosign sign: 출력 이미지 서명 (레지스트리 push 필요 → 로컬 레지스트리 경유)
#   [4/4] trivy: Critical 0건 게이트 (exit-code 1)
#
# 인수조건 1: Critical 1건 이상 시 exit 1
# ──────────────────────────────────────────────────────────
build-secure-crypto: ## [Step 1-B] crypto-engine: Cosign verify(WARN) → build → sign → Trivy
	@which cosign > /dev/null 2>&1 || { echo "Error: cosign 미설치. 'make setup' 실행."; exit 1; }
	@which trivy  > /dev/null 2>&1 || { echo "Error: trivy 미설치."; exit 1; }
	@echo "==> [build-secure-crypto 1/4] Cosign: 베이스 이미지 서명 확인 (python:3.12-alpine, WARN-only)"
	@cosign verify python:3.12-alpine > /dev/null 2>&1 \
		&& echo "==> [build-secure-crypto 1/4] PASS ✓ — Cosign 서명 확인" \
		|| echo "==> [build-secure-crypto 1/4] WARN — python:3.12-alpine 미서명 (Docker Hub 기본, 허용)"
	@echo "==> [build-secure-crypto 2/4] docker build: crypto-engine"
	@DOCKER_CONTENT_TRUST=0 docker compose build crypto-engine
	@echo "==> [build-secure-crypto 3/4] Cosign: 출력 이미지 서명/검증 (임시 로컬 레지스트리 경유)"
	@if [ -f .cosign/cosign.key ]; then \
		...
		if docker run -d --name pqc-registry -p 5000:5000 registry:2 \
			...
			docker rm -f pqc-registry 2>/dev/null || true; \
			echo "==> [build-secure-crypto 3/4] PASS ✓ — 출력 이미지 서명 검증 통과"; \
		else \
			docker rm -f pqc-registry 2>/dev/null || true; \
			echo "==> [build-secure-crypto 3/4] FAIL — 서명 실패 (.cosign/cosign.key 또는 Docker 상태 확인)"; \
			exit 1; \
		fi; \
	else \
		echo "==> [build-secure-crypto 3/4] SKIP — .cosign/cosign.key 없음 (make cosign-keygen 실행 권장)"; \
	fi
	@echo "==> [build-secure-crypto 4/4] Trivy: Critical 취약점 스캔 (exit-code 1)"
	@trivy image --severity CRITICAL --exit-code 1 pqc-security-project-crypto-engine
	@echo "==> [build-secure-crypto] 완료 ✓"

# ──────────────────────────────────────────────────────────
# build-secure-gateway / build-secure-dashboard
#   Phase 2/3 완성 후 실제 파이프라인으로 교체
# ──────────────────────────────────────────────────────────
build-secure-gateway: ## [Step 1-C] api-gateway: Phase 2 완성 후 활성화 예정 (SKIP)
	@echo "==> [build-secure-gateway] SKIP — api-gateway는 Phase 2 완성 후 파이프라인 교체 예정"

build-secure-dashboard: ## [Step 1-C] dashboard: Phase 3 완성 후 활성화 예정 (SKIP)
	@echo "==> [build-secure-dashboard] SKIP — dashboard는 Phase 3 완성 후 파이프라인 교체 예정"

# ──────────────────────────────────────────────────────────
# build-secure — Step 1-C: 4개 서비스 순차 보안 파이프라인
#   db → crypto-engine → api-gateway(SKIP) → dashboard(SKIP)
#
# 인수조건 2: 4개 서비스 순서 실행, exit 0
# ──────────────────────────────────────────────────────────
build-secure: ## [Step 1-C] 4서비스 순차 보안 파이프라인 (db→crypto→gateway→dashboard)
	@which cosign > /dev/null 2>&1 || { echo "Error: cosign 미설치. 'make setup' 실행."; exit 1; }
	@which trivy  > /dev/null 2>&1 || { echo "Error: trivy 미설치."; exit 1; }
	@echo "==> [build-secure] 4개 서비스 순차 파이프라인 시작 ─────"
	@echo "==> [build-secure 1/4] db"
	@$(MAKE) build-secure-db
	@echo "==> [build-secure 2/4] crypto-engine"
	@$(MAKE) build-secure-crypto
	@echo "==> [build-secure 3/4] api-gateway"
	@$(MAKE) build-secure-gateway
	@echo "==> [build-secure 4/4] dashboard"
	@$(MAKE) build-secure-dashboard
	@echo "==> [build-secure] 모든 서비스 완료 ✓"
...
# ──────────────────────────────────────────────────────────
# test-db — Step 1-C DB 스키마 인수조건 검증
#   테이블 4개 + 인덱스 6개(idx_*) + su-exec uid=70 확인
#   전제: docker compose up db 완료 상태
# ──────────────────────────────────────────────────────────
test-db: ## [Step 1-C] DB 스키마 검증 (테이블4+인덱스6+su-exec uid=70)
